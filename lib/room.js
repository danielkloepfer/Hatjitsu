...
Room.prototype.enter = function(socket, data) {
  ...
  if (this.connections[data.sessionId]) {
    ...
    this.connections[data.sessionId] = { sessionId: data.sessionId, socketId: socket.id, vote: null, voter: true, voterName: null };
  }
}
...
Room.prototype.changeVoterName = function(data) {
  if (this.connections[data.sessionId]) {
    this.connections[data.sessionId]['voterName'] = data.voterName ? data.voterName.slice(0,7) : null;
    // console.log("voter name set to " + data.voterName + " for " + data.sessionId);
  }
  this.io.sockets.in(this.roomUrl).emit('voter status changed');
}
